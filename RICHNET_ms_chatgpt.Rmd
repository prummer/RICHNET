...
For each gene set cluster, the member gene set names are provided to a Gemini model (via the Generative Language API) with the instruction to return a single concise 5-word label that best describes the common biological theme of the cluster. The R code below calls the Generative Language API (model `gemini-1.5`) and reads the API key from the environment variable `GEMINI_API_KEY`. The resulting 5-word labels are assigned to `cl3.lab.txt` and used exactly as before for plot titles. If `GEMINI_API_KEY` is not set, labels default to NA and a warning is emitted.

```{r chatgpt_labels, echo=F}
# Prepare cluster member text for Gemini and call Generative Language API per cluster
# Try system env first, then fall back to a .env file in the working directory
gemini_key <- Sys.getenv("GEMINI_API_KEY")
if (nchar(gemini_key) == 0) {
  env_file <- "C:/Users/mprummer/Documents/_Michael/nexus_github/RICHNET/.env"
  if (file.exists(env_file)) {
    env_lines <- readLines(env_file, warn = FALSE)
    env_lines <- env_lines[grepl("=", env_lines)]
    if (length(env_lines) > 0) {
      kv <- strsplit(env_lines, "=", fixed = TRUE)
      env_list <- setNames(lapply(kv, function(x) paste(x[-1], collapse = "=")), sapply(kv, function(x) trimws(x[1])))
      if ("GEMINI_API_KEY" %in% names(env_list)) gemini_key <- trimws(env_list[["GEMINI_API_KEY"]])
    }
  }
}
if (nchar(gemini_key) == 0) {
  warning("GEMINI_API_KEY not set: cluster labels will be NA. Set GEMINI_API_KEY in your environment or in a .env file in the working directory to enable Gemini labeling.")
  cl3.lab.txt <- rep(NA, max(clu3$membership))
} else {
  library(httr)
  library(jsonlite)
  # cluster members: one string per cluster with readable names
  cluster_members <- tapply(names(clu3$membership), clu3$membership, function(x) paste(gsub("_", " ", x), collapse = "\n"))
  # helper to query ChatGPT for a single cluster
  get_label <- function(members_text) {
    prompt <- paste0(
      "You are given a list of gene set names belonging to a single cluster.\n",
      "Provide ONE concise label of exactly five words that best describes the common biological theme of these gene sets.\n",
      "Return only the label with no surrounding punctuation or quotation marks.\n\n",
      members_text
    )
    body <- list(
    ...
    res <- POST(
          url = paste0("https://generativelanguage.googleapis.com/v1beta2/models/gemini-1.5:generateText?key=", URLencode(gemini_key, reserved = TRUE)),
          add_headers(`Content-Type` = "application/json"),
      body = toJSON(body, auto_unbox = TRUE)
    )
    if (status_code(res) >= 400) {
      warning("Generative Language API request failed: ", content(res, as = "text"))
      return(NA)
    }
    resp_parsed <- content(res, as = "parsed", simplifyVector = TRUE)
    txt <- NA
    if (!is.null(resp_parsed$candidates) && length(resp_parsed$candidates) > 0) {
      if (!is.null(resp_parsed$candidates[[1]]$content)) txt <- resp_parsed$candidates[[1]]$content
      else if (!is.null(resp_parsed$candidates[[1]]$output)) txt <- resp_parsed$candidates[[1]]$output
    } else if (!is.null(resp_parsed$choices) && length(resp_parsed$choices) > 0 && !is.null(resp_parsed$choices[[1]]$message$content)) {
      txt <- resp_parsed$choices[[1]]$message$content
    }
    if (is.null(txt) || length(txt) == 0) return(NA)
    txt <- gsub("[\r\n]+", " ", txt)
    txt <- trimws(txt)
    # keep only first five words if model returned more
    words <- unlist(strsplit(txt, "\\s+"))
    if (length(words) == 0) return(NA)
    out <- paste(words[1:min(5, length(words))], collapse = " ")
    return(out)
  }
  # Query ChatGPT for each cluster (slow for many clusters)
  cl_ids <- sort(unique(clu3$membership))
  cl3.lab.txt <- sapply(cl_ids, function(id) {
    members <- paste(rownames(data.frame(cl = clu3$membership))[which(clu3$membership == id)], collapse = "\n")
    Sys.sleep(0.5) # brief pause to be polite to the API
    get_label(gsub("_", " ", members))
  })
}
```
...
There are many possiblities to visualize geneset clusters and often a compromize between information content and crowding has to be found. Here, we are producing a lattice of network plots, one for each sub-net, with the automatic annotation as title. We begin by ensuring cluster titles are available in `cl3.lab.txt` and then clean up and order by cluster size. 

```{r}
clust = data.frame(cl = clu3$membership)
rownames(clust) = names(V(net3))
# cl3.lab.txt is expected to be provided by the ChatGPT chunk above. If missing, fall back to NA.
if(!exists("cl3.lab.txt") || length(cl3.lab.txt) < max(clu3$membership)) {
  cl3.lab.txt <- rep(NA, max(clu3$membership))
}
# remove NAs and keep ordering
...
Then we generate a list of ggplot objects, one for each cluster or sub-net. For smaller sub-nets, the nodes are labelled with the first 4 words of their names; the first word was removed before as it is usually the name of the geneset library. For larger sub-nets, this is not feasible without overprinting. Titles are missing if no label could be obtained.
...
for (ii in 1:max(clu3$membership)) {
  ...
  # use ChatGPT label (converted to uppercase) with one optional line break
