...
This version replaces the previous word-frequency based automatic labeling with a call to ChatGPT. Plan: for each gene set cluster, the member gene set names are provided to ChatGPT with the instruction to return a single concise 5-word label that best describes the common biological theme of the cluster. The R code below calls the OpenAI Chat Completions API (model `gpt-4.1`) and reads the API key from the environment variable `OPENAI_API_KEY`. The resulting 5-word labels are assigned to `cl3.lab.txt` and used exactly as before for plot titles. If `OPENAI_API_KEY` is not set, labels default to NA and a warning is emitted.

```{r chatgpt_labels, echo=F}
# Prepare cluster member text for ChatGPT and call OpenAI API per cluster
...
if (nchar(api_key) == 0) {
  ...
  if (file.exists(env_file)) {
    ...
    if (length(env_lines) > 0) {
      ...
      if ("OPENAI_API_KEY" %in% names(env_list)) api_key <- trimws(env_list[["OPENAI_API_KEY"]])
    }
  }
}
if (nchar(api_key) == 0) {
  warning("OPENAI_API_KEY not set: cluster labels will be NA. Set OPENAI_API_KEY in your environment or in a .env file in the working directory to enable ChatGPT labeling.")
  cl3.lab.txt <- rep(NA, max(clu3$membership))
} else {
  library(httr)
  library(jsonlite)
  # cluster members: one string per cluster with readable names
  cluster_members <- tapply(rownames(data.frame(cl = clu3$membership)), cl = clu3$membership, FUN = function(x) paste(gsub("_", " ", x), collapse = "\n"))
  ...
  get_label <- function(members_text) {
    ...
    if (status_code(res) >= 400) {
      warning("OpenAI API request failed: ", content(res, as = "text"))
      return(NA)
    }
    txt <- content(res, as = "parsed", simplifyVector = TRUE)$choices[[1]]$message$content
    txt <- gsub("[\r\n]+", " ", txt)
    txt <- trimws(txt)
