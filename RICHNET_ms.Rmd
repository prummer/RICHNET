...
<!--
1. General introductory remarks / context / what's this all about.
2. State of the art / what is known / what do we have.
3. What's missing / unmet need / open question.
4. What is the current work about and how is it solving the problem.
5. What is the benefit to science / mankind / the universe.
6. Preview the structure and content of the remainder of the paper.
-->
...
The packages required for this workflow provide plotting functions (`ggplot` and relatives), network functions (`igraph`, `GGally`), text analytics functions (`wordcloud`, etc.) and gene expression analysis functions (`DESeq2`, `org.Hs.eg.db`, `limma`).
...
lby = c("RColorBrewer", "ggplot2", "gplots", "cowplot", 
        ...
        "GGally", "igraph", "DESeq2", "limma", "org.Hs.eg.db", 
...
We are using the popular _airway_ data set and perform a simple differential expression analysis.
...
tmp.dir = paste0(home.dir, "/tmp")
gset.file = paste0(tmp.dir, "/human_c2_v5p2.rdata")
if(file.exists(gset.file)){
  temp.space = new.env()
  bar = load(gset.file, temp.space)
  ...
  gset = get(bar, temp.space)
  rm(temp.space)
  shell(paste0("mkdir ", tmp.dir), translate = T)
  save(gset, file = gset.file)
}

## uncomment this chunk for real submission & delete previous chunk.  
# url = "http://bioinf.wehi.edu.au/software/MSigDB/human_c2_v5p2.rdata"
# temp.space = new.env()
# bar = load(url(url), temp.space)
# gset = get(bar, temp.space)
