...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{home.dir =}\StringTok{ }\KeywordTok{Sys.getenv}\NormalTok{(}\StringTok{"HOME"}\NormalTok{)}
\NormalTok{tmp.dir =}\StringTok{ }\KeywordTok{paste0}\NormalTok{(home.dir, }\StringTok{"/tmp"}\NormalTok{)}
\NormalTok{gset.file =}\StringTok{ }\KeywordTok{paste0}\NormalTok{(tmp.dir, }\StringTok{"/human_c2_v5p2.rdata"}\NormalTok{)}
\ControlFlowTok{if}\NormalTok{(}\KeywordTok{file.exists}\NormalTok{(gset.file))\{}
\NormalTok{  temp.space =}\StringTok{ }\KeywordTok{new.env}\NormalTok{()}
\NormalTok{  bar =}\StringTok{ }\KeywordTok{load}\NormalTok{(gset.file, temp.space)}
...
\NormalTok{  gset =}\StringTok{ }\KeywordTok{get}\NormalTok{(bar, temp.space)}
  \KeywordTok{rm}\NormalTok{(temp.space)}
  \KeywordTok{shell}\NormalTok{(}\KeywordTok{paste0}\NormalTok{(}\StringTok{"mkdir "}\NormalTok{, tmp.dir), }\DataTypeTok{translate =}\NormalTok{ T)}
  \KeywordTok{save}\NormalTok{(gset, }\DataTypeTok{file =}\NormalTok{ gset.file)}
\NormalTok{\}}

\NormalTok{## uncomment this chunk for real submission & delete previous chunk.  }
\CommentTok{# url = "http://bioinf.wehi.edu.au/software/MSigDB/human_c2_v5p2.rdata"}
\CommentTok{# temp.space = new.env()}
\CommentTok{# bar = load(url(url), temp.space)}
\CommentTok{# gset = get(bar, temp.space)}
\CommentTok{# rm(temp.space)}
...
\NormalTok{  v.label =}\StringTok{ }\KeywordTok{sapply}\NormalTok{(v.label, }\ControlFlowTok{function}\NormalTok{(x) }\KeywordTok{paste}\NormalTok{(x[}\DecValTok{2}\OperatorTok{:}\KeywordTok{min}\NormalTok{(}\DecValTok{5}\NormalTok{, }\KeywordTok{length}\NormalTok{(x))], }\DataTypeTok{collapse =} \StringTok{"_"}\NormalTok{))}
