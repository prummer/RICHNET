...
```{r gemini_annotation}
# Load necessary packages for API interaction and environment variables
if (!requireNamespace("httr", quietly = TRUE)) install.packages("httr"); library(httr)
if (!requireNamespace("jsonlite", quietly = TRUE)) install.packages("jsonlite"); library(jsonlite)

# Load .env file (assuming it's in the current working directory)
...
if (file.exists(env_file)) {
  ...
  if (length(env_lines) > 0) {
    ...
    if ("GEMINI_API_KEY" %in% names(env_list)) GEMINI_API_KEY <- trimws(env_list[["GEMINI_API_KEY"]])
  }
}
...
if (GEMINI_API_KEY == "") {
  stop("GEMINI_API_KEY environment variable not found. Please set it in your .env file or system environment.")
}

# Define the Gemini API endpoint
GEMINI_ENDPOINT = "https://generativelanguage.googleapis.com/v1beta/models/gemini-pro-latest:generateContent"

# Function to call Gemini API for cluster annotation
gemini_annotate_cluster = function(gene_set_names) {
  ...
  prompt_text = paste0(
    "Given the following gene sets: ",
    paste(gene_set_names, collapse = ", "),
    ". Provide a 5-word label that best describes their common biological theme."
  )
  ...
  response = POST(
    url = paste0(GEMINI_ENDPOINT, "?key=", GEMINI_API_KEY),
  ...
  if (http_error(response)) {
    warning(paste("Gemini API request failed with status:", status_code(response)))
    return("API Error: Could not get label")
  }
  ...
  if (!is.null(parsed_response$candidates[[1]]$content$parts[[1]]$text)) {
    label = parsed_response$candidates[[1]]$content$parts[[1]]$text
    # Ensure it's approximately 5 words (Gemini is usually good at this, but for safety)
    words = strsplit(label, "\\s+")[[1]]
    label = paste(words[1:min(5, length(words))], collapse = " ")
    return(label)
  } else {
    warning("Gemini API response did not contain a valid label.")
